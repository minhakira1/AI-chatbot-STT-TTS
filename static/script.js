...
const getSpeechToText = async (userRecording) => {
  ...
  formData.append("audio", userRecording.audioBlob, "recorded.wav");
  ...
  if (response.text && response.text.trim() !== "") {
    ...
    return "[Speech recognition error: " + response.error + "]";
  } else {
...
const processUserMessage = async (userMessage) => {
  let response = await fetch(baseUrl + "/process-message", {
    ...
    body: JSON.stringify({ text: userMessage, voice: voiceOption }),
  });
  response = await response.json();
  console.log(response);
  return response;
...
const playResponseAudio = (function () {
  ...
  return function Sound(src) {
    console.log('Playing audio src:', src);
...
const populateBotResponse = async (userMessage) => {
  ...
  $("#message-list").append(
    `<div class='message-line'><div class='message-box${
      ...
      response.text
    }</div><button id='${repeatButtonID}' class='btn volume repeat-button' onclick='playResponseAudio("data:audio/mp3;base64," + responses[botRepeatButtonIDToIndexMap[this.id]].audio);console.log(this.id)'><i class='fa fa-volume-up'></i></button></div>`
  );

  playResponseAudio("data:audio/mp3;base64," + response.audio);
    ...
    if (!sttRecording) {
      ...
      $.ajax({
        ...
        success: function (data) {
          $("#stt-result").val(data.text);
        },
        error: function () {
          $("#stt-result").val("Error transcribing audio.");
        },
      });
    }
  });
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.audio) {
          ...
          $("#tts-audio").attr("src", audioSrc).show()[0].play();
        }
      },
      error: function () {
